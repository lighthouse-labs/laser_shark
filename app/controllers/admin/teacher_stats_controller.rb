class Admin::TeacherStatsController < Admin::BaseController
  ...
  def index
    ...
    feedback_sub = Feedback.teacher_feedbacks.completed.group('1').select('feedbacks.teacher_id, COUNT(feedbacks.id) AS feedback_count, ROUND(AVG((feedbacks.style_rating::numeric + feedbacks.technical_rating::numeric) / 2.0)::numeric, 2)::float AS average_feedback_score').to_sql
  ...
  def feedback
    avg_col = "ROUND(AVG((style_rating::numeric + technical_rating::numeric) / 2.0), 2)::float"
